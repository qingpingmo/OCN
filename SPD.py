...
def compute_adj2_with_shortest_paths(Ej, adj):
    
    adj2 = spspmm(Ej, 1, adj, 0)
    num_nodes = adj2.shape[0]

    adj_indices = adj.to_torch_sparse_coo().indices()  
    adj_values = adj.to_torch_sparse_coo().values()    

    adj2_indices = adj2.to_torch_sparse_coo().indices()  
    adj2_values = adj2.to_torch_sparse_coo().values()   

    adj2_indices = adj2_indices[:, adj2_indices[0] < num_nodes]  
    adj2_values = adj2_values[adj2_indices[0] < num_nodes]  

    adj_mask = torch.ones_like(adj2_values, dtype=torch.bool)  

    for i in range(adj_indices.shape[1]):  
        
        mask_condition = (adj2_indices[0] == adj_indices[0][i]) & (adj2_indices[1] == adj_indices[1][i])
        adj_mask = adj_mask & ~mask_condition 

    adj2_values = adj2_values * adj_mask.int()

    adj2 = SparseTensor(row=adj2_indices[0], col=adj2_indices[1], value=adj2_values, sparse_sizes=(num_nodes, num_nodes))
    
    row, col, val = adj2.coo()
    if val is None:
        val = torch.ones_like(row, dtype=torch.float)
    adj2 = SparseTensor(torch.stack((row, col)), val, adj2.sizes(), is_coalesced=True)
...
def get_cn1_cn2(adj,tedge):
    ...
    Ej2 =compute_adj2_with_shortest_paths(Ej,adj)
    del Ej
